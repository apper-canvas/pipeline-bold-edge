...
if (searchTerm) {
      filtered = filtered.filter(activity => {
        ...
        return (
          ...
          (contact && contact.name_c?.toLowerCase().includes(searchTerm.toLowerCase())) ||
...
if (activeFilters.contact) {
      filtered = filtered.filter(activity => {
        const contactId = activity.contact_id_c?.Id || activity.contact_id_c
        const contact = contacts.find(c => c.Id === contactId)
        return contact && contact.name_c?.toLowerCase().includes(activeFilters.contact.toLowerCase())
      })
    }
...
activities.forEach(activity => {
      const date = new Date(activity.timestamp_c).toDateString()
      if (!groups[date]) {
        groups[date] = []
      }
